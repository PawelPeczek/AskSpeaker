...
CREATE TABLE IF NOT EXISTS `AskSpeakerDB`.`Users` (
  ...
  PRIMARY KEY (`UserID`, `UserRoles_UserRoleID`),
  UNIQUE INDEX `UserID_UNIQUE` (`UserID` ASC),
  UNIQUE INDEX `UserName_UNIQUE` (`UserName` ASC),
  INDEX `fk_Users_UserRoles_idx` (`UserRoles_UserRoleID` ASC),
...
DELIMITER $$
USE `AskSpeakerDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`Votes_BEFORE_INSERT` BEFORE INSERT ON `Votes` FOR EACH ROW
BEGIN
...
USE `AskSpeakerDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`Votes_BEFORE_UPDATE` BEFORE UPDATE ON `Votes` FOR EACH ROW
BEGIN
  IF (NEW.Value NOT IN (-1, 1)) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'invalid data';
  END IF;
